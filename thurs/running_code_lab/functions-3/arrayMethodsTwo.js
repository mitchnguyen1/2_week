...
// CODE HERE
const filly = cities.filter(el => el.charAt(el.length-1) === "i")
console.log(filly)
////////// PROBLEM 4 //////////
...
/* 
    ...
    print "The fruit with index [THE ELEMENT INDEX] is an orange." If the color is not red or apple, print 
...
// CODE HERE
//filter out the objects with rice in the tags array
let total = foods.filter(el => el.tags.includes("rice"))
//add the total together by adding the price only, curr only need property and 0 for initial val if accessing obj's prop
let final = total.reduce((prev,curr) => prev + curr.price,0)
let oneLine = foods.filter(el => el.tags.includes("rice")).reduce((prev,curr) => prev + curr.price,0)
// THE TOTAL
console.log(oneLine)
