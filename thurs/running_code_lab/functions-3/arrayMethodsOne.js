...
//fancy version below, makes it a string though
...
//          orders.price=15 * orders.tax=0.09 + 1         
...
// CODE HERE
function randomFunct(orders){
  let newOrder = []
  for(let i = 0; i < orders.length; i++){
    newOrder.push(orders[i].price * (orders[i].tax +1))
  }
  return newOrder;
}

const randomMap = orders => orders.map(el => el.price * (el.tax + 1))
console.log(randomMap(orders))
...
// CODE HERE
//filter out and get bob's price
let filterPurchases = purchases.filter(purchases => purchases.owner === "Bob").reduce((acc,curr) => acc + curr.price, 0)

//sum bobs orders
// let sum = filterPurchases.reduce((acc,curr) => acc + curr.price, 0)


//return a grand total as integer
console.log(`Total Sum: $${filterPurchases}`)
